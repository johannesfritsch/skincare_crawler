...
| `source-reviews` | Individual reviews per retailer variant (sourceVariant ref → source-variants, externalId unique dedup key, rating 0-10 normalized, title, reviewText, userNickname, submittedAt, reviewerAge, reviewerGender, isRecommended, positiveFeedbackCount, negativeFeedbackCount). Ratings are normalized to a 0-10 scale regardless of source (e.g. BazaarVoice 1-5 stars × 2). |
| `brands` | Brand names |
