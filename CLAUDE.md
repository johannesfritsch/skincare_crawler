...
│   └── CLAUDE.md               # Payload CMS development rules & patterns
...
│   └── CLAUDE.md               # Worker architecture & internals
...
- See `server/CLAUDE.md` for Payload CMS development patterns
...
See `worker/CLAUDE.md` for full worker architecture, job types, data flow, and internals.
...
| `recognition_embeddings` | Standalone pgvector table for DINOv2-small (384-dim) embeddings keyed by (product_variant_id, detection_media_id). Decoupled from Payload's recognitionImages array to survive array rewrites. HNSW index for cosine similarity search. |
...
8. Video processing runs as 8 sequential stages (scene_detection → barcode_scan → object_detection → visual_search → llm_recognition → transcription → compile_detections → sentiment_analysis), each persisting immediately. Detection data is stored on video-scenes in tabbed arrays (barcodes[], objects[], recognitions[], llmMatches[], detections[]). Video-frames are pure frame records (no detection data). Transcription is per-scene: extracts full audio, then for each scene extracts an audio clip via `ffmpeg -ss -t`, transcribes the clip individually via Whisper API, and LLM-corrects each scene's text; transcripts are stored only on video-scenes (no video-level transcript field). compile_detections synthesizes all detection sources into a unified detections[] array with confidence scores (barcode: 1.0, DINOv2: 1.0-distance, LLM: 0.6, multi-source bonus: +0.1 per additional source, capped at 1.0). sentiment_analysis concatenates scene transcripts on-the-fly to derive the full transcript context, then creates video-mentions with full detection provenance (confidence, sources, barcodeValue, clipDistance). Progress is tracked per-video on the job's `videoProgress` JSON map (not on the video). Job selects which stages to run via checkboxes. The download stage has been moved to the video crawl step.
```
