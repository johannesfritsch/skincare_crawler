...
| `source-reviews` | Individual reviews per retailer product (sourceProduct ref → source-products required, sourceVariants hasMany ref → source-variants optional, reviewOrigin ref → review-origins optional, externalId unique dedup key, rating 0-10 normalized, title, reviewText, userNickname, submittedAt, reviewerAge, reviewerGender, isRecommended, positiveFeedbackCount, negativeFeedbackCount). Reviews are owned by the source-product (product-level); sourceVariants tracks which variants the review was encountered on (grows as more variants are crawled). Ratings are normalized to a 0-10 scale regardless of source (e.g. BazaarVoice 1-5 stars × 2). `reviewOrigin` links to the syndication source (e.g. "Home Tester Club") when the review came from a BazaarVoice SyndicationSource; null for native store reviews. |
| `source-review-origins` | Syndication sources for reviews, deduplicated per (name, source) pair. Fields: `name` (text, required, indexed), `source` (select: dm/rossmann/mueller/purish, required), `incentivized` (checkbox, nullable — null means unclassified), `reasoning` (textarea, LLM explanation). Compound unique index on (name, source). Created automatically during review persistence when a BazaarVoice `SyndicationSource.Name` is first encountered; classified by gpt-4.1-mini as incentivized or not on creation. Examples: "Home Tester Club" (incentivized), "The Insiders" (incentivized), brand-owned shops (not incentivized). |
| `brands` | Brand names |
