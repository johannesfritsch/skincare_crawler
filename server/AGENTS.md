...
| `ProductVideoList` | `components/product-video-list.tsx` | Client | Paginated video mention cards with sentiment badge overlay on thumbnail, creator avatar, timestamp, and optional featured quote strip (top quote by sentiment score). Links include `?snippetId=X` to deep-link into video detail. Props: `videos: ProductVideoItem[]`. Exports `ProductVideoItem` type (includes `snippetId`, `topQuote`, `topQuoteSentiment`). |
