...
| 2 | `screenshot_detection` | Per snippet: Grounding DINO detection on cluster representative screenshots, crop + upload to detection-media, store in `detections` array. Configurable: `detectionPrompt` (default "cosmetics packaging."), `detectionThreshold` (default 0.3), `minBoxArea` (default 25%). Emits per-candidate detail events for full observability. | `video-snippets.detections` (crops as detection-media, bounding boxes) |
| 3 | `screenshot_search` | Per snippet: CLIP ViT-B/32 transient embeddings for detection crops → cosine search vs product embeddings → match products. Configurable: `searchThreshold` (default 0.3, try 0.5-0.7 for video), `searchLimit` (default 1). Emits per-detection detail events with top-N distances for threshold calibration. | `video-snippets.detections` (match info), `video-snippets.referencedProducts` |
