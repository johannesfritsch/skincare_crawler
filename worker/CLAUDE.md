...
| `persistReviews()` | Standalone review persistence — find-or-create source-reviews by `externalId`, link variants. When a review has a `reviewSource` (BazaarVoice SyndicationSource), finds-or-creates a `source-review-origins` record by (name, source) and links the source-review to it via `reviewOrigin` relationship. New origins are classified by gpt-4.1-mini as incentivized or not on creation (with reasoning). Uses an in-memory cache to avoid repeated DB queries within one invocation. Race conditions on the compound unique index are handled via catch-and-retry. Accepts `source` (SourceSlug) and optional `jlog` parameters for origin creation and event emission. |
...
- `transcriptionModel` (text field, default: 'whisper-1') — free-form text since local OpenAI-compatible servers may use different model names
...
| 9 | `review_sentiment` | `stageReviewSentiment` | GPT 4.1-mini analysis of source-reviews → per-topic sentiment counts → upsert product-sentiments with reviewOrigin dimension (post-LLM origin tagging). Incremental via `reviewState` (skips already-processed reviews). Fresh runs (reviewState=0) delete existing sentiments to prevent double-counting. Configurable: `reviewSentimentChunkSize` (default 50). | Yes |
| 10 | `sentiment_conclusion` | `stageSentimentConclusion` | Algorithmic conclusions from product-sentiments per origin group (All/Incentivized/Organic/individual origins). Dynamic thresholds: 5 for aggregate groups, 3 for individual origins. Upserts product-sentiment-conclusions with groupType + reviewOrigin. No LLM. | No |
...
interface ScrapedProductData {
  ...
  warnings: string[]
  reviews?: Array<{ externalId: string; rating: number; title?: string; reviewText?: string; userNickname?: string; submittedAt?: string; reviewerAge?: string; reviewerGender?: string; isRecommended?: boolean; positiveFeedbackCount?: number; negativeFeedbackCount?: number }>
}
```
...
| `analyzeSentiment(transcript, products, fullTranscript?)` | scene transcript + product info + optional full transcript | `{ products[]: { quotes[], overallSentiment, score }, tokensUsed }` | `handleVideoProcessing` |
