...
| `product-variants` | Per-variant records for unified products. Each has: `product` (required relationship → products, indexed), `gtin` (unique, indexed), `label`, `image` (upload → media), `sourceVariants` (hasMany relationship → source-variants), `recognitionImages` (array of {image: upload → media, score: number, boxXMin/boxYMin/boxXMax/boxYMax: number} — Grounding DINO detection crops). Aggregation pipeline creates these when creating/updating products. Frontend routes via `/products/[gtin]` look up product-variants first. Frontend list queries use `min(gtin)` + `GROUP BY product.id` to deduplicate (one row per product). |
...
| `product-aggregations` | type (all/selected_gtins), stage checkboxes (stageResolve, stageClassify, stageMatchBrand, stageIngredients, stageImages, stageObjectDetection, stageDescriptions, stageScoreHistory), language, imageSourcePriority, includeSisterVariants (default true — groups sibling GTINs sharing a source-product into one product with multiple variants), aggregationProgress (JSON map of progressKey → last completed stage, e.g. `{ "4012345678901": "classify", "pid:4012345678901": "42" }`), aggregated/errors/tokens |
