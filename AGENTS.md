...
│   ├── src/
│   │   ├── payload.config.ts
│   │   ├── collections/        # 24 Payload collection configs
...
| `source-categories` | Category trees from individual retailers (slug, source, parent ref, url) |

### Video Data

| Collection | Purpose |
|------------|---------|
| `videos` | YouTube/social videos (channel ref, title, duration, processingStatus) |
| `video-snippets` | Video segments with product refs (timestamps, matchingType: barcode/visual, screenshots) |
...
| `category-discoveries` | storeUrls, progress |
| `ingredients-discoveries` | sourceUrl, currentTerm/Page, termQueue |
| `video-discoveries` | channelUrl, created/existing counts |
| `video-processings` | type (all_unprocessed/single_video/selected_urls), processed/errors/tokens |
