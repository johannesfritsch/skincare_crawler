...
| `source-products` | Raw crawled data per retailer (status: uncrawled/crawled/failed, source: dm/mueller/rossmann/purish, sourceUrl: unique product page URL — the dedup key, name, brand, images, categoryBreadcrumb text, ingredientsText raw textarea, priceHistory with change: drop/stable/increase, rating, ratingNum, description). No longer has `gtin` — GTINs live on `source-variants`. Has a `sourceVariants` join to source-variants collection. Discovery/search creates source-products with `sourceUrl` but no variants — variants are only created during crawl. |
| `source-variants` | Per-variant records for retailer products. Each has: `sourceProduct` (required relationship → source-products, indexed), `sourceUrl` (unique, indexed), `gtin` (indexed), `variantLabel`, `variantDimension`, `availability` (select: available/unavailable/unknown, default unknown — set from stock data during crawl), `crawledAt` (date, tracks when this specific variant was last crawled). Created only during product crawl (not during discovery/search). For DM/Rossmann, the crawled product URL itself becomes a variant. For Mueller, only `?itemId=` URLs are variants (the base URL lives on source-products.sourceUrl, not as a variant). For PURISH, only `?variant=ID` URLs are variants. |
| `brands` | Brand names |
