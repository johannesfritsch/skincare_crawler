...
| `source-variants` | Per-variant records for retailer products. Each has: `sourceProduct` (required relationship → source-products, indexed), `sourceUrl` (unique, indexed — the dedup key), `gtin` (indexed), `variantLabel`, `variantDimension`, `availability` (select: available/unavailable/unknown, default unknown — set from stock data during crawl), `crawledAt` (date, tracks when this specific variant was last crawled). Every source-product has at least one source-variant. For Mueller, only `?itemId=` URLs are kept as variants (base URL variant is deleted once itemId variants are discovered during crawl). Mueller's `scrapeProduct` extracts all variants (including unavailable ones) from the Next.js RSC payload (`siblings[]` with `stockLevel`), not from DOM tiles. |
| `brands` | Brand names |
